...
- An auto-deployed namespace + Ingress at
  \`https://<slug>.projects.corpo-valley.com\`, gated at the edge by the Kratos
...
Each project has:
- A Gitea repo at \`gitea.corpo-valley.com/<owner>/<slug>\`
...
- A live URL \`https://<slug>.projects.corpo-valley.com\`.
...
- \`Build\` — \`docker build\` the repo's Dockerfile, push to
  \`registry.cv-registry.svc.cluster.local:5000/<owner>/<slug>\` tagged
  with the build timestamp \`YYYYMMDDHHMMSS\` (immutable) and the short
  SHA. Then it calls the Corpo Valley portal (in-cluster URL
  \`portal.cv-portal.svc.cluster.local/internal/projects/<slug>/pin\`),
...
\`/api\`, \`/mcp\`) — all wired to the project's namespace, the cv-registry
image path, and the \`<slug>.projects.corpo-valley.com\` Ingress host with the
...
\`corpo-valley.com\` so the same session covers \`portal\`, \`auth\`,
\`oauth\`, \`gitea\`, and every \`<slug>.projects.corpo-valley.com\`.
